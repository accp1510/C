...
STud a[size], b={"Тучка","Малявка",{2,3,3,4,5}};

int StZach (STud [], int);
int HBall(STud *);
int StOtl (STud [], int);
void RaChar (STud [], int);
...
void main()
{
	...
	srand(time(NULL));
		for (i=0;i<size;i++)																		//Вводим структуру
			{
				RaChar(a,i);																		//псевдослучайные числа
				for(j=0;j<5;j++)
					{
...
void RaChar (STud m[], int p)																		//псевдослучайные буквы
{
	int fr, nr, i;
	char alf[12]={'s','v','e','b','q','w','m','t','h','a','n','p'};
	for(i=0;i<3;i++)
	{
		fr=rand()%12;
		nr=rand()%12;
